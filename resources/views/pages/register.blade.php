...
@section('content')
    <div class="flex justify-center items-center w-full">
        <x-card class="w-full max-w-md p-8 shadow-2xl border-t-4 border-[#67bed9]">
            <div class="text-center mb-8">
                <h2 class="text-3xl font-bold text-gray-900">{{ $title }}</h2>
                <p class="text-gray-500 mt-2">{{ $description }}</p>
            </div>

            <form method="POST" action="{{ route('register') }}" class="space-y-6" x-data @submit.prevent="$dispatch('notify', 'We\'re still working on it. Please bear with us!');">
                @csrf

                <div>
                    <x-label for="name" value="Name" />
                    <x-input id="name" class="block mt-1 w-full focus:border-[#67bed9] focus:ring-[#67bed9]" type="text" name="name" :value="old('name')" required autofocus />
                </div>

                <div>
                    <x-label for="email" value="Email" />
                    <x-input id="email" class="block mt-1 w-full focus:border-[#67bed9] focus:ring-[#67bed9]" type="email" name="email" :value="old('email')" required />
                </div>

                <div x-data="{ showPassword: false }">
                    <x-label for="password" value="Password" />
                    <div class="relative mt-1">
                        <x-input id="password" class="block w-full focus:border-[#67bed9] focus:ring-[#67bed9] pr-10" type="password" ::type="showPassword ? 'text' : 'password'" name="password" required autocomplete="new-password" />
                ...
                <div x-data="{ showConfirmPassword: false }">
                    ...
                    <div class="relative mt-1">
                        <x-input id="password_confirmation" class="block w-full focus:border-[#67bed9] focus:ring-[#67bed9] pr-10" type="password" ::type="showConfirmPassword ? 'text' : 'password'" name="password_confirmation" required />
            ...
            <div class="mt-6 text-center">
                <p class="text-sm text-gray-600">
                    ...
                    <a href="{{ route('login') }}" class="font-medium text-[#67bed9] hover:text-[#afd374] transition-colors duration-300">
                        Login
                    </a>
                </p>
            </div>
